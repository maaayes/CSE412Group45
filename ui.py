...
class CreateAlbum:
    def __init__(self, master):
        ...
        self.album_entry_label = tk.Label(master, text="Album name:", font=("Helvetica", 14))
        self.album_entry_label.pack(pady=10)
        
        self.ftext_box = tk.Text(master, width=30,height=3)
        self.ftext_box.pack()
        album_name = self.ftext_box.get(1.0, 'end-1c')
        
        self.button = tk.Button(master, text="Back", command=self.back)
        self.button.pack(pady=10)
        
        self.save_button = tk.Button(master, text="Save", command=self.upload_album, bg='#f0f0f0', width=15)
    ...
    def upload_album(self):
        album_id = random.randint(14300,2877777)
        current_date = datetime.datetime.now().strftime("%Y-%m-%d")
        dba.insert_album(conn, album_id, album_name, userID, current_date)
...
class FriendFeed:
    def __init__(self, master):
        ...
        self.button = tk.Button(master, text="Add or Remove Friends", command=self.AddRemove)
        self.button.pack(pady=5)
    ...
    def AddRemove(self):
        self.master.withdraw()  
        self.newWindow = tk.Toplevel(self.master)  
        self.app = AddRemove(self.newWindow) 
...
class AddRemove:
    def __init__(self, master):
        self.master = master
        master.geometry("500x500")
        master.title("Add or Remove Friends")
        
        self.htlabel = tk.Label(master, text="Friend's username:")
        self.htlabel.pack()
        self.httext_box = tk.Text(master, width=30,height=3)
        self.httext_box.pack()
        
        save_button = tk.Button(master, text="Add Friend", command=self.addfriend, bg='#f0f0f0', width=15)
        save_button.pack(pady=10)
    ...
    def addfriend(self):
        current_date = datetime.datetime.now().strftime("%Y-%m-%d")
        dba.insert_friend(conn, user_id, friend_id, current_date)
